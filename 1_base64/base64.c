 ...
 */
long int encodeToBase64(unsigned char *buffer, char *base64Buffer, long int *fileSize, int *padding) {
...
int main(int argc, char **argv) {
	...
	*/ 
	...
	/**
	* point data of string to buffer
	*/  
		buffer = argv[2];
	...
	*/ 
	...
	long int endPointer = encodeToBase64(buffer, base64Buffer, &fileSize, &padding);
	
	if(strcmp(argv[1], "string") == 0) {
		base64Buffer[endPointer] = '\0';
	}
	...
	if(strcmp(argv[1], "file") == 0) {
		...
		newFilename = concatenateString(newFilename, argv[2], (char *)".b64");
